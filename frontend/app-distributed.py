...
@app.route('/certs')
def certs_page():
    return send_from_directory('/export/html', 'certs.html')

# --- Certificate Management API ---
CERT_DIR = os.environ.get('SHELLAMA_CERT_DIR', '/etc/shellama/pki')
...
def list_certs():
    ...
    for f in sorted(os.listdir(CERT_DIR)):
        if f.endswith('-cert.pem'):
            ...
            try:
                info = _run_openssl(['x509', '-in', path, '-noout', '-subject', '-enddate', '-serial'])
                lines = info.strip().split('\n')
                subject = lines[0].replace('subject=', '').strip() if lines else ''
                expires = lines[1].replace('notAfter=', '').strip() if len(lines) > 1 else ''
                serial = lines[2].replace('serial=', '').strip() if len(lines) > 2 else ''
                certs.append({'file': f, 'subject': subject, 'expires': expires, 'serial': serial})
            except:
                certs.append({'file': f, 'subject': 'error reading', 'expires': '', 'serial': ''})
    has_ca = os.path.exists(os.path.join(CERT_DIR, 'ca-cert.pem'))
    return jsonify({'certs': certs, 'has_ca': has_ca, 'cert_dir': CERT_DIR})

@app.route('/api/certs/generate-ca', methods=['POST'])
def generate_ca():
    ...
    if os.path.exists(ca_cert):
        return jsonify({'error': 'CA already exists. Delete it first to regenerate.'}), 400
    data = request.json or {}
    org = data.get('org', 'sheLLaMa')
    cn = data.get('cn', 'sheLLaMa-CA')
    days = data.get('days', 3650)
    try:
        _run_openssl(['genrsa', '-out', ca_key, '4096'])
        _run_openssl(['req', '-new', '-x509', '-days', str(days), '-key', ca_key, '-out', ca_cert,
                      '-subj', f'/O={org}/CN={cn}'])
        os.chmod(ca_key, 0o600)
        # Init CA database
        open(os.path.join(CERT_DIR, 'index.txt'), 'a').close()
        with open(os.path.join(CERT_DIR, 'serial'), 'w') as f:
            f.write('1000\n')
        with open(os.path.join(CERT_DIR, 'crlnumber'), 'w') as f:
            f.write('1000\n')
        return jsonify({'status': 'ok', 'file': 'ca-cert.pem'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/certs/generate', methods=['POST'])
def generate_cert():
    """Generate a server or client certificate signed by the CA."""
    _ensure_cert_dir()
    ca_cert = os.path.join(CERT_DIR, 'ca-cert.pem')
    ca_key = os.path.join(CERT_DIR, 'ca-key.pem')
    if not os.path.exists(ca_cert):
        return jsonify({'error': 'No CA exists. Generate a CA first.'}), 400
    data = request.json or {}
    name = data.get('name', '')
    cert_type = data.get('type', 'server')  # server or client
    sans = data.get('sans', '')  # comma-separated DNS names for server certs
    days = data.get('days', 3650)
    if not name:
        return jsonify({'error': 'Name is required'}), 400
    safe_name = name.replace(' ', '-').replace('/', '-')
    cert_file = os.path.join(CERT_DIR, f'{safe_name}-cert.pem')
    key_file = os.path.join(CERT_DIR, f'{safe_name}-key.pem')
    req_file = os.path.join(CERT_DIR, f'{safe_name}-req.pem')
    if os.path.exists(cert_file):
        return jsonify({'error': f'Certificate {safe_name} already exists'}), 400
    try:
        _run_openssl(['genrsa', '-out', key_file, '4096'])
        _run_openssl(['req', '-new', '-key', key_file, '-out', req_file,
                      '-subj', f'/O=sheLLaMa/CN={name}'])
        sign_args = ['x509', '-req', '-days', str(days), '-in', req_file,
                     '-CA', ca_cert, '-CAkey', ca_key, '-CAcreateserial', '-out', cert_file]
        if cert_type == 'server' and sans:
            ext_file = os.path.join(CERT_DIR, f'{safe_name}-ext.cnf')
            san_list = ','.join(f'DNS:{s.strip()}' for s in sans.split(',') if s.strip())
            with open(ext_file, 'w') as f:
                f.write(f'subjectAltName = {san_list}\n')
            sign_args += ['-extfile', ext_file]
        _run_openssl(sign_args)
        os.chmod(key_file, 0o600)
        return jsonify({'status': 'ok', 'cert': f'{safe_name}-cert.pem', 'key': f'{safe_name}-key.pem'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def revoke_cert():
    ...
    _ensure_cert_dir()
    data = request.json or {}
    cert_file = data.get('file', '')
    if not cert_file or '..' in cert_file:
    ...
    ca_cert = os.path.join(CERT_DIR, 'ca-cert.pem')
    ca_key = os.path.join(CERT_DIR, 'ca-key.pem')
    ca_conf = os.path.join(CERT_DIR, 'ca.conf')
    with open(ca_conf, 'w') as f:
        f.write(f'[ca]\ndefault_ca = CA_default\n[CA_default]\ndir = {CERT_DIR}\n'
                f'database = $dir/index.txt\nserial = $dir/serial\ncrlnumber = $dir/crlnumber\n'
                f'default_crl_days = 30\ndefault_md = sha256\n')
    try:
        _run_openssl(['ca', '-config', ca_conf, '-revoke', cert_path,
                      '-keyfile', ca_key, '-cert', ca_cert])
        _run_openssl(['ca', '-config', ca_conf, '-gencrl',
                      '-keyfile', ca_key, '-cert', ca_cert,
                      '-out', os.path.join(CERT_DIR, 'crl.pem')])
        return jsonify({'status': 'ok', 'revoked': cert_file})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def delete_cert():
    """Delete a certificate and its key/req files."""
    data = request.json or {}
    name = data.get('name', '')
    if not name or '..' in name or name == 'ca':
        return jsonify({'error': 'Invalid name'}), 400
    deleted = []
    for suffix in ['-cert.pem', '-key.pem', '-req.pem', '-ext.cnf']:
        path = os.path.join(CERT_DIR, f'{name}{suffix}')
